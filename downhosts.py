...
# Takes a list of IPs and returns a list of IPs addresses that are down.
...
async def tf_ping(host:str) -> bool:
    ...
    if type(response) is float:
        return True # async3.ping returns responsetime floats for successful pings
    if response is None:
        return False # async3.ping returns None for timeouts
    if response is False:
        return response  # async3.ping returns False for resolution errors
